...
- Обнаружение всех отдельных UCAMS-камер аккаунта как самостоятельных privacy-safe устройств Home Assistant.
...
## Текущий релиз и кандидат v0.32.0
...
Текущая release-ветка — **кандидат v0.32.0** с live-проверенной поддержкой отдельных UCAMS-камер, объединённой через PR #17. Синхронизированная версия кандидата ещё не является тегом или опубликованным релизом; для публикации нужны exact-head CI и отдельное явное разрешение.
...
Для кандидата v0.32.0 URL основного ресурса с соответствующим cache-bust:
...
### Отдельные UCAMS-камеры (кандидат v0.32.0)

Если в аккаунте есть UCAMS-камеры, не привязанные к домофону, интеграция создаёт для них отдельные устройства Home Assistant. Та же конфигурация `custom:ufanet-intercom-card` может указывать на live-сущность отдельной камеры. В таком режиме карточка оставляет только **LIVE** и **АРХИВ** и скрывает управление дверью, звонки, гостей, авторизованные устройства, физические ключи и диагностику домофона.

Для отдельной камеры поддерживаются live HLS, JPEG-снимки, позиция и навигация по архиву, события/метки движения, ручной MP4-экспорт и медиатека экспортов. Home Assistant получает только стабильную opaque-ссылку камеры; provider camera number, media tokens, server domains и raw analytics остаются внутри интеграции.

Реализация PR #17 live-проверена 15 сентября 2026 года: обнаружение без дубля домофонной камеры, live-видео, снимки, переходы и управление архивом, motion/timeline, MP4 export/library, обновления Lovelace и сохранение состояния после полного рестарта Home Assistant прошли без замечаний. Это подтверждение относится к протестированному аккаунту и планируется для v0.32.0; в опубликованный релиз v0.31.0 оно не входит.
