...
- `external_id` — строковый идентификатор, который официальный Android-клиент использует как `filters.key` при запросе истории выбранного ключа;
...
Validation-сервисы управления используют локальный opaque `key_ref` вместо provider `id`. `list_physical_keys` дополнительно выводит **значение** `external_id` как экспериментальное пользовательское поле `number`. Это удобно для визуального сравнения, но проект **пока не подтвердил**, что оно совпадает с номером, физически нанесённым на брелок. Такое соответствие должно оставаться **Experimental** до проверки заведомо известного ключа.

`external_id` остаётся приватным wire/runtime identifier для фильтрации истории. Реальные номера ключей не включаются в diagnostics, logs, events, публичные support bundles или примеры репозитория.
...
Этот контракт live-подтверждён на реальном зарегистрированном ключе с двумя возвращёнными проходами.
...
Добавляйте поле в этот документ только если оно действительно наблюдалось в ответе API или коде клиента. Неясную семантику помечайте явно вместо догадки. Не публикуйте в примерах реальные provider key IDs, реальные значения `external_id`/номеров ключей, идентификаторы камер/событий или сырую историю конкретного аккаунта.
