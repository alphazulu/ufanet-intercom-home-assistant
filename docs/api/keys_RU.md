...
Внутренний provider ID не показывается. Opaque `key_ref` используется только внутри вызовов HA service и пользователю не выводится.
...
Важное отличие от decompiled Android DTO: DTO объявляет `key` как integer, а реальный backend отдаёт JSON string. Gson в Android принимает numeric string для integer field; Home Assistant теперь явно зеркалирует это поведение.
