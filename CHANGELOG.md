...
- Live-confirmed the non-empty physical-key model returned by `/api/v4/key/list/`: `id`, `external_id`, `name`, `create_date`, and `devices`. Internal provider `id` remains private. The value of `external_id` is exposed only as an **Experimental** user-facing `number` candidate because correspondence to the digits printed on the physical key is not yet proven.
...
- Added a validation-only **КЛЮЧИ / KEYS** Lovelace tab as a separate packaged frontend extension. It shows key name, Experimental key-number candidate, creation time, selected-key passage history, the same-device enrollment control, and backend-verified rename. Internal provider IDs, raw `external_id` field names, and delete actions are not rendered.
...
- Updated EN/RU user documentation, notification guide, physical-key reference, FCM reference, API verification matrix, data models, security guidance and publishing checklist to distinguish Confirmed, Observed, Inferred and Experimental behavior and preserve the required live-validation gates.
