...
- Added **Add physical key** for intercoms that advertise `has_key_recording_support`. The `POST /api/v4/key/skud/<id>/auto_collect/enable/` path and real 60-second enrollment window are now live-confirmed end to end with a genuinely unregistered physical key; HTTP dispatch alone is still not treated as proof of registration.
- Added `reason=key_add` FCM completion handling, immediate physical-key coordinator refresh, privacy-safe `ufanet_intercom_key_enrollment` events, and token/identifier-free key-add diagnostics. Real headless-FCM completion was live-confirmed after new-key enrollment; the tested success matched `key_status == 0` plus a parseable `key_id`. A separate no-key 60-second timeout produced no additional completion push.
