...
- Added validation-only `list_physical_keys` and `rename_physical_key` response services. The list exposes only an entry/intercom-scoped opaque `key_ref`, `name`, and `created_at`; rename refreshes inventory before resolving the ref, invokes the Android-observed edit contract internally, and requires a second inventory refresh to confirm the requested name before reporting verified success. The provider rename endpoint remains **Observed** until a controlled live rename is performed.
...
- Registered the main Lovelace card and packaged extensions through the race-safe Lovelace resource path to prevent intermittent **Configuration error** after dashboard reloads.
...
- Kept production history loading on the already live-confirmed `POST /api/v0/analytics/motion_alarm/report/` contract with safe `start`/`end` window splitting; the Android-observed `/api/v0/analytics/archive_events/` endpoint remains documented as Observed only and is not used by production runtime.
