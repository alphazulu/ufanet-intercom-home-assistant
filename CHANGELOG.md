...
- Live-confirmed the non-empty physical-key model returned by `/api/v4/key/list/`: `id`, `external_id`, `name`, `create_date`, and `devices`. Internal provider `id` remains private. The value of `external_id` is exposed only as an **Experimental** user-facing `number` candidate because correspondence to the digits printed on the physical key is not proven; the current release work explicitly keeps that mapping Experimental rather than claiming it Confirmed.
- Added validation-only `list_physical_keys` and `rename_physical_key` response services. The list exposes an entry/intercom-scoped opaque `key_ref`, Experimental `number`, `name`, and `created_at`; rename refreshes inventory before resolving the ref, invokes the Android-observed edit contract internally, and requires a second inventory refresh to confirm the requested name before reporting verified success. The provider rename endpoint remains **Observed** until a controlled live rename is performed.
- Added live-confirmed per-key passage-history support through `get_physical_key_passages`. The real API returns passage `key` as a numeric JSON string, and the official Android flow filters a selected key with `filters.key=<external_id>`; the Home Assistant parser mirrors Gson-compatible coercion and the public service returns only normalized passage timestamps. A live key selection returned two passages end to end in Lovelace.
- Added a validation-only **КЛЮЧИ / KEYS** Lovelace tab as separate packaged frontend extensions. It shows key name, visibly Experimental key-number candidate, creation time, selected-key passage history, the same-device enrollment control, and backend-verified rename. Internal provider IDs, raw `external_id` field names, and delete actions are not rendered.
...
- Updated EN/RU user documentation, notification guide, physical-key reference, FCM reference, API verification matrix, data models, security guidance, contribution guidance and publishing checklist to distinguish Confirmed, Observed, Inferred and Experimental behavior and preserve the required live-validation gates.
