...
- Added validation-only `list_physical_keys` and `rename_physical_key` response services. The list exposes an entry/intercom-scoped opaque `key_ref` rather than provider key IDs; rename refreshes inventory before resolving that ref, invokes the Android-observed `POST /api/v4/key/edit/` contract internally, and requires a second inventory refresh to confirm the requested name before reporting verified success. The provider rename endpoint remains **Observed** until a real key is tested.
- Added a validation-only **КЛЮЧИ / KEYS** Lovelace tab as a separate packaged frontend extension. It uses only the privacy-safe list/rename services and the same-device Home Assistant enrollment button, shows the 60-second enrollment countdown, refreshes inventory afterward, requires backend-verified rename success, never renders provider key IDs/external IDs, and deliberately contains no delete action. The empty-state layout and browser integration remain a live visual gate until tested on the development dashboard.
- Kept the Android-observed physical-key delete contract (`POST /api/v4/key/skud/<id>/delete/key/`) documented but unimplemented. Delete remains explicitly destructive and outside the current release scope.
