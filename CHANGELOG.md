...
- Added `reason=key_add` FCM completion handling using the Android-observed success rule (`key_status == 0` plus a valid internal key identifier), an immediate physical-key coordinator refresh, privacy-safe `ufanet_intercom_key_enrollment` events, and token/identifier-free key-add diagnostics. The real `key_add` wire payload remains Observed until a new physical key is available for end-to-end testing.
- Extended the **Physical keys** sensor with a read-only `keys` inventory filtered to the selected intercom. Public rows contain only `name` and UTC `created_at`; private provider identifiers are discarded or retained only internally as required. The empty live state (`state=0`, `keys=[]`) has been verified in Home Assistant; non-empty inventory remains a release gate.
- Added validation-only `list_physical_keys` and `rename_physical_key` response services. The list exposes an entry/intercom-scoped opaque `key_ref` rather than provider identifiers; rename refreshes inventory before resolving that ref, invokes the Android-observed edit contract internally, and requires a second inventory refresh to confirm the requested name before reporting verified success. The zero-key list response (`count=0`, `keys=[]`) has been live-tested; the provider rename endpoint remains **Observed** until a real key is tested.
- Added a validation-only **КЛЮЧИ / KEYS** Lovelace tab as a separate packaged frontend extension. It uses only the privacy-safe list/rename services and the same-device Home Assistant enrollment button, shows the 60-second enrollment countdown, refreshes inventory afterward, requires backend-verified rename success, never renders provider identifiers, and deliberately contains no delete action. The empty-state layout and browser integration remain a live visual gate until tested on the development dashboard.
