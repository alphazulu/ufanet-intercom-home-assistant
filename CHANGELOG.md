...
- Fixed Android Companion payload compatibility discovered during live testing (`trigger.event.context.id` rather than a missing bare context and string `authenticationRequired="true"` for the FCM data channel). Android real-call delivery, physical door opening, direct live-camera navigation and timeout replacement of the same notification have been live-tested. A second real call was also confirmed to supersede the first pending action, fresh notification metadata matched the expected device/location/local time, and a successful **Open door** action replaced the same notification without leaving a stale door action or creating a duplicate. The unavailable negative live test with a second Ufanet device was explicitly waived after targeted security/code review; only that live test is waived, while the cross-device safety invariant and runtime guards remain in force. The Android notification block has no remaining hard release gate.
- Added canonical authorized-device response services `list_authorized_devices`, `revoke_authorized_device` and `revoke_other_authorized_devices`, plus separate advanced FCM cleanup services `list_fcm_registrations`, `unregister_fcm_registration` and `unregister_other_fcm_registrations`. The historical `*_fcm_sessions` names remain compatibility aliases; their revoke actions still call `logout_device`.
- Live-confirmed cross-session Ufanet authorization revocation from a plain `auth_by_contract` controller that had no FCM registration: `logout_device` removed a different test-device row, the target's already-issued access JWT remained accepted immediately afterward, the target refresh JWT was rejected with HTTP 401, and the independent controller/observer remained authorized.
- Live-confirmed direct `DELETE /api/v0/fcm/` without calling `logout_device` against a disposable probe-owned registration: the target row disappeared from `authorized_devices`, the target's existing access JWT remained temporarily usable, the target refresh JWT was rejected with HTTP 401, the independent observer remained authorized, and the registration was restored with a fresh JWT login. The advanced FCM action is therefore documented as authorization-destructive for the tested refresh chain rather than as a harmless push unsubscribe.
- Corrected the `authorized_devices` model: a plain JWT session can read it, but live FCM unregister testing proved that rows are coupled to device/FCM registration state, so it is treated as the provider device/registration inventory used by the official active-device UI rather than an exhaustive independent list of all JWT sessions.
- Updated the **УСТРОЙСТВА / DEVICES** Lovelace path to use canonical authorized-device services for normal revoke operations and a separate collapsed advanced FCM section for direct unregister. Public UI uses opaque `authorization_ref` / `fcm_ref`, protects locally provable Home Assistant registrations, and never renders raw provider IDs or FCM tokens. The updated UI was live-tested with successful removal of test entries while the Home Assistant-owned registration remained protected.
...
- Hardened `scripts/release_check.py` for 0.31.0 preparation: all packaged frontend extension cache-bust values must now match the manifest/integration/card/README version, every packaged frontend JavaScript file is parsed with `node --check`, and response-service references from extensions are checked against `services.yaml`. This closes a release-gate gap where an extension could otherwise retain an older cache-bust while the self-check stayed green.
...
- Updated EN/RU user documentation, authentication/FCM references, API verification matrix, security guidance, notification guide, physical-key reference, contribution/publishing guidance and draft release notes to preserve evidence labels, record the authorization/FCM target-token tests, record the negative printed-number test, document live-confirmed rename/eventual consistency, record the notification cross-device live-test waiver, and retain only the remaining new-key enrollment live-validation gates.
...
- This work remains **validation-only**: do not tag or publish a release until the active combined-validation PR's remaining physical-key enrollment/real-`reason=key_add` gates are completed or explicitly reviewed/waived.
