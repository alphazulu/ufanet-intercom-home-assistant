...
- Added validation-only `list_physical_keys` and `rename_physical_key` response services. The list exposes an entry/intercom-scoped opaque `key_ref` plus user-facing `number` derived from the key's live-confirmed `external_id`, while the internal provider `id` remains hidden. Rename refreshes inventory before resolving that ref, invokes the Android-observed edit contract internally, and requires a second inventory refresh to confirm the requested name before reporting verified success. The provider rename endpoint remains **Observed** until a controlled live rename is performed.
- Added live-confirmed per-key passage-history support through `get_physical_key_passages`. The real API returns passage `key` as a numeric JSON string, and the official Android flow filters a selected key with `filters.key=<external_id>`; the Home Assistant parser now mirrors Gson-compatible coercion and the public service returns only normalized passage timestamps. A live key selection returned two passages end to end in Lovelace.
- Added a validation-only **КЛЮЧИ / KEYS** Lovelace tab as a separate packaged frontend extension. It shows key name, physical-key number, creation time, selected-key passage history, the same-device enrollment control, and backend-verified rename. Internal provider IDs, raw `external_id` field names, and delete actions are not rendered.
