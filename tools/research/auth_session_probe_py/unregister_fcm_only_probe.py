"""Controlled FCM-only unregister probe using ordinary Ufanet JWT auth."""
...
async def refresh_tokens(
    ...
    print(f"[OK] Controller JWT refresh after FCM unregister: HTTP {status}")
    return tokens
...
def print_inventory(devices: list[AuthorizedDevice], *, label: str) -> None:
...
async def run(args: argparse.Namespace) -> int:
    ...
    print(
        "[INFO] FCM-only separation test. The controller is a plain Ufanet JWT login. "
        "This script does NOT import Firebase/FCM libraries, does NOT obtain or register "
        "an FCM token, does NOT start MCS, and NEVER calls logout_device. The only push "
        "mutation is DELETE /api/v0/fcm/ for the selected target."
    )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        await test_controller_access(
            session,
            controller.access,
            label="controller access before FCM unregister",
        )

        before = await get_authorized_devices(session, controller.access)
        print_inventory(before, label="Authorized devices BEFORE FCM unregister")
        ...
        print(
            f"[INFO] Selected exactly: title={target.title!r} "
            f"session_ref={target.session_ref}"
        )
        if not args.confirm:
            raise ProbeError(
                "FCM unregister was not performed; rerun with --confirm after checking the target"
            )

        await unregister_fcm_only(session, controller.access, target)

        after = await get_authorized_devices(session, controller.access)
        print_inventory(after, label="Authorized devices AFTER FCM unregister")
        still_present = any(device.device_id == target.device_id for device in after)
        print(
            "[RESULT] target_present_in_authorized_devices_after_fcm_delete="
        ...
        if still_present:
            print(
                "[CONCLUSION] DELETE /api/v0/fcm/ removed the selected push registration "
                "without removing its authorized-device record. This is direct evidence "
                "that FCM unregister and Ufanet authorization revoke are separate actions."
            )
        else:
            print(
                "[CONCLUSION] The selected row disappeared from authorized_devices after "
                "FCM-only DELETE even though logout_device was never called. This proves "
                "the inventory is coupled to FCM registration state, but by itself does NOT "
                "prove that the target JWT authorization was revoked. Check the target app "
                "while it remains offline before allowing it to re-register FCM."
            )
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled Ufanet FCM-only unregister test that never calls logout_device"
        )
    )
    parser.add_argument("--contract", help="Ufanet contract/login")
    target = parser.add_mutually_exclusive_group(required=True)
    target.add_argument("--title", help="Exact authorized-device title to FCM-unregister")
    target.add_argument("--session-ref", help="Opaque auth-XXXXXXXXXXXX reference shown by this probe")
    parser.add_argument(
        "--confirm",
        action="store_true",
        help="Required to perform DELETE /api/v0/fcm/",
    )
    parser.add_argument(
