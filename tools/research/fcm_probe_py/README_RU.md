...
Чтобы live-проверить наблюдавшийся в Android-клиенте контракт удаления FCM-регистрации:
...
заново. Флаг не удаляет другие устройства и не выводит `device_id` или token. Если
повторная регистрация не удалась, достаточно снова запустить `py probe.py` без флага.

До успешного live-теста компонент Home Assistant намеренно не использует DELETE в
рабочем lifecycle.
